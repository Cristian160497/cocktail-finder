...
export const useCocktailStore = create((set, get) => ({ 
  ...
  // Applica filtri combinati - MODIFICATO per accettare parametri opzionali
  applyFilters: async (overrides = {}) => { 
    const state = get();
    
    // Usa i valori passati come parametri, altrimenti usa quelli nello state
    const selectedCategory = overrides.category !== undefined ? overrides.category : state.selectedCategory;
    const selectedIngredient = overrides.ingredient !== undefined ? overrides.ingredient : state.selectedIngredient;
    const selectedAlcoholic = overrides.alcoholic !== undefined ? overrides.alcoholic : state.selectedAlcoholic;
    const searchTerm = overrides.searchTerm !== undefined ? overrides.searchTerm : state.searchTerm;
